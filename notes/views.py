...
class NotesHomepageView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['selected_to_delete'] = NotesModel.objects.filter(checkbox=True)
        return context
...
class DeleteAllChosenNote(ListView):
    queryset = NotesModel.objects.filter(checkbox = True)
...
def checkbox_delete_check(request, pk):
    note = get_object_or_404(NotesModel, id=pk)
    note.checkbox = not note.checkbox
    print(note.checkbox)
    note.save()
    return redirect('index')
